  ...
  try {
    ...
    if (userVault === hre.ethers.ZeroAddress) {
      ...
      try {
        ...
        const tx = await factory.connect(user).createVault(swapRouter, wrappedNative, bot.address);
    ...
    if (!hasOracleRole) {
      ...
      if (adminRoleHolder === user.address) {
        ...
        const tx = await vault.connect(user).grantRole(ORACLE_ROLE, bot.address);
        await tx.wait();
    ...
    console.log(`尝试存入 ${hre.ethers.formatEther(depositAmount)} 原生代币...`);
    
    try {
      const tx = await vault.connect(user).depositNative({ value: depositAmount });
      await tx.wait();
      console.log("存入原生代币成功");
      
      // 检查余额
      nativeBalance = await vault.getBalance(NATIVE_TOKEN);
      console.log(`存入后原生代币余额: ${hre.ethers.formatEther(nativeBalance)}`);
    } catch (error) {
    ...
    if (nativeBalance > 0) {
      ...
      console.log(`尝试提取 ${hre.ethers.formatEther(withdrawAmount)} 原生代币...`);
      ...
      try {
        ...
        // 检查余额
        nativeBalance = await vault.getBalance(NATIVE_TOKEN);
        console.log(`提取后原生代币余额: ${hre.ethers.formatEther(nativeBalance)}`);
      } catch (error) {
    ...
    if (!TOKEN_A_ADDRESS || !TOKEN_B_ADDRESS) {
      console.log("⚠️ 跳过swap测试: 缺少TOKEN_A_ADDRESS或TOKEN_B_ADDRESS环境变量");
      return;
    }
    ...
    console.log(`尝试存入 ${hre.ethers.formatEther(tokenDepositAmount)} TokenA到金库...`);
    ...
    try {
      ...
      // 存入代币
      const tx = await vault.connect(user).deposit(TOKEN_A_ADDRESS, tokenDepositAmount);
      await tx.wait();
      ...
      if (tokenABalance > 0) {
        ...
        console.log(`Swap前余额: TokenA=${hre.ethers.formatEther(balA_before)}, TokenB=${hre.ethers.formatEther(balB_before)}`);
        ...
        try {
          ...
          console.log(`尝试swap: ${hre.ethers.formatEther(swapAmount)} TokenA -> TokenB`);
          ...
          console.log(`Swap后余额: TokenA=${hre.ethers.formatEther(balA_after)}, TokenB=${hre.ethers.formatEther(balB_after)}`);
        } catch (error) {
