...
async function main() {
    ...
    // 费用参数
    const feeRecipient = process.env.FEE_RECIPIENT || ethers.ZeroAddress; // 收费人地址
    const feeRate = Number(process.env.FEE_RATE) || 0; // 费率，默认0（无费用）
    ...
    console.log(`费用收取人: ${feeRecipient}`);
    console.log(`费率: ${feeRate} (${feeRate / 10000}%)`);
    ...
    // 使用UniV2版本的swap函数，增加费用参数
    const tx = await Vault.swapExactInputSingle(
        tokenIn,
        tokenOut,
        amountIn,
        amountOutMin,
