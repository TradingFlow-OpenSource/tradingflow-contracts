...
async function main() {
  ...
  try {
    ...
    try {
      ...
      // 获取当前nonce
      const nonce = await provider.getTransactionCount(user.address);
      ...
      try {
        ...
        console.log(`估算gas失败: ${error.message}`);
        console.log("使用固定gas限制");
        gasEstimate = 5000000;
      }
      ...
      console.log(`交易已提交: ${txResponse.hash}`);
