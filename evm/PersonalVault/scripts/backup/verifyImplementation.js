...
async function main() {
  ...
  try {
    ...
    // 尝试获取实现合约的初始化函数选择器
    const initSelector = ethers.id("initialize(address,address,address,address,address)").substring(0, 10);
    ...
    // 创建工厂合约实例
    const factoryWithCreateVault = new ethers.Contract(factoryAddress, createVaultAbi, provider);
    ...
    try {
      // 尝试估算gas
      console.log(`尝试估算createVault(${swapRouter}, ${wrappedNative}, ${botAddr})的gas...`);
