 ...
 *      - Swap Router: 0x13f4EA83D0bd40E75C8222255bc855a974568Dd4 (PancakeSwap V3)
...
contract PersonalVaultFactoryV1 is Ownable, AccessControl {
     ...
     *      - Admin (vaultAdmin): Has ADMIN_ROLE for settings (setRouterV2, etc.)
     *      - Bot (botAddress): Has ORACLE_ROLE for executing trades
     *      - Investor (msg.sender): Can deposit/withdraw funds
     * @param swapRouter The address of the swap router (recommended: 0x13f4EA83D0bd40E75C8222255bc855a974568Dd4)
     * @param wrappedNative The address of the wrapped native token (WBNB: 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c)
    ...
    /**
     * @notice Set V2 Router for all vaults
     * @dev Batch operation to set V2 Router address for all existing vaults
     * @param _routerV2 The PancakeSwap V2 Router address
     */
    function setRouterV2ForAllVaults(address _routerV2) external onlyRole(ADMIN_ROLE) {
        require(_routerV2 != address(0), "Invalid V2 router");
        
        for (uint i = 0; i < allVaults.length; i++) {
            PersonalVaultUpgradeableV1 vault = PersonalVaultUpgradeableV1(payable(allVaults[i]));
            vault.setRouterV2(_routerV2);
        }
    }
    
    /**
     * @notice Set V2 Router for a specific vault
     * @param vaultAddress The vault address
     * @param _routerV2 The PancakeSwap V2 Router address
     */
    function setRouterV2ForVault(address vaultAddress, address _routerV2) external onlyRole(ADMIN_ROLE) {
        require(vaultAddress != address(0), "Invalid vault");
        require(_routerV2 != address(0), "Invalid V2 router");
        
        PersonalVaultUpgradeableV1 vault = PersonalVaultUpgradeableV1(payable(vaultAddress));
        vault.setRouterV2(_routerV2);
    }
}
