...
async function main() {
  ...
  try {
    ...
    console.log(
      `条件1 - 用户没有金库: ${
        userVault === ethers.ZeroAddress ? "通过" : "失败 - 用户已有金库"
      }`
    );
    ...
    console.log(
      `条件2 - 实现合约已设置: ${
        implAddress !== ethers.ZeroAddress ? "通过" : "失败 - 未设置实现合约"
      }`
    );
    ...
    console.log(
      `条件4 - Bot地址有效: ${
        botAddr !== ethers.ZeroAddress ? "通过" : "失败 - 无效的Bot地址"
      }`
    );
    ...
    console.log(
      `条件5 - Bot有BOT_ROLE权限: ${
        hasBotRole ? "通过" : "失败 - Bot没有BOT_ROLE权限"
      }`
    );
    ...
    console.log(
      `条件6 - 用户有足够余额支付Gas: ${
        userBalance > ethers.parseEther("0.01") ? "通过" : "失败 - 余额不足"
      }`
    );
    ...
    console.log(
      `swapRouter: ${swapRouter} - ${
        swapRouter === "0xf45AFe28fd5519d5f8C1d4787a4D5f724C0eFa4d"
          ? "匹配"
          : "不匹配"
      }`
    );
    console.log(
      `wrappedNative: ${wrappedNative} - ${
        wrappedNative === "0xd3bF53DAC106A0290B0483EcBC89d40FcC961f3e"
          ? "匹配"
          : "不匹配"
      }`
    );
    console.log(
      `botAddr: ${botAddr} - ${
        botAddr === "0x1227c9ED38A2D99f443FF143c36238cedf9B45af"
          ? "匹配"
          : "不匹配"
      }`
    );
