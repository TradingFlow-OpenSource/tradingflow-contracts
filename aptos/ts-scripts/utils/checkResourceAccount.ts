...
async function checkResourceAccount() {
  try {
    ...
    const resourceCapability = resources.find((r) =>
      r.type.includes("ResourceSignerCapability")
    );

    if (resourceCapability) {
      console.log(
        "找到资源账户能力:",
        JSON.stringify(resourceCapability, null, 2)
      );
      ...
      // 查询 Version 资源
      const versionResource = resources.find((r) => r.type.includes("Version"));
      ...
      // 查询 AccessList 资源
      const accessListResource = resources.find((r) =>
        r.type.includes("AccessList")
      );

      if (accessListResource) {
        console.log(
          "访问控制列表:",
          JSON.stringify(accessListResource, null, 2)
        );
      }
    ...
    console.log(
      "2. 如果找到资源账户能力但缺少 FungibleStore，请先执行存款操作创建 FungibleStore"
    );
 ...
 */
async function checkFungibleStore(
  aptos: Aptos,
  ...
  try {
    ...
    const matchingStore = fungibleStores.find((r) =>
      JSON.stringify(r).includes(metadataId)
    );

    if (matchingStore) {
      console.log(
        `  ✅ 找到 ${tokenName} 代币的 FungibleStore:`,
        JSON.stringify(matchingStore, null, 2)
      );
    } else {
      console.log(`  ❌ 未找到 ${tokenName} 代币的 FungibleStore`);
      console.log(
        `  建议: 执行存款操作来创建 ${tokenName} 代币的 FungibleStore`
      );
    }
  } catch (error) {
